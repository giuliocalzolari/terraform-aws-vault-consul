...
data "template_file" "vault" {
  ...
  vars = {
    ...
    consul_url   = "https://releases.hashicorp.com/consul/${var.consul_version}/consul_${var.consul_version}_linux_${local.arch_version[var.arch]}.zip"
    aws_region   = var.aws_region
    arch_version = local.arch_version[var.arch]
    cluster_size = var.size
    uuid         = random_uuid.env.result
    app_name     = "${var.prefix}${var.app_name}${var.suffix}"
    environment  = var.environment
    gossip_pwd   = random_id.gossip_key.hex
  }
}
...
resource "aws_autoscaling_group" "asg" {
  ...
  tag {
    ...
    value               = "${var.environment}-${var.prefix}${var.app_name}${var.suffix}-${random_uuid.env.result}"
    propagate_at_launch = true
  }
  ...
  target_group_arns     = [aws_alb_target_group.main.arn, aws_alb_target_group.consul.arn]
  lifecycle {
    create_before_destroy = true
  }
}
