...
if [[ ! -z $${YUM} ]]; then
  ...
  logger "Setting up user consul for amazon_linux"
  user_rhel "consul" "consul" "/etc/consul" "Hashicorp consul user"
  ...
  echo "Setting up user consul for Debian/Ubuntu"
  user_ubuntu "consul" "consul" "/etc/consul" "Hashicorp consul user"
...
curl --silent --output /tmp/consul.zip ${consul_url}
unzip -o /tmp/consul.zip -d /sbin/
...
User=consul
Group=consul
ExecStart=/sbin/consul agent -config-dir=/etc/consul/
ExecReload=/sbin/consul reload
ExecStop=/sbin/consul leave
KillMode=process
Restart=on-failure
LimitNOFILE=65536
...
cat << EOF >  /etc/consul/consul.json
{
  ...
  "node_name": "$INSTACE_ID",
  "client_addr": "0.0.0.0",
  "datacenter": "${aws_region}-${environment}-${app_name}-${uuid}",
  "server": true,
  ...
  "retry_join": ["provider=aws tag_key=environment_name tag_value=${environment}-${app_name}-${uuid}"]
}
EOF
...
echo "waiting consul boot"
waitforurl http://127.0.0.1:8500/v1/status/leader
