...
resource "aws_lb_listener_rule" "consul_rule" {
  ...
  action {
    type = "redirect"
    redirect {
      host        = "${var.prefix}${var.app_name2}${var.suffix}.${data.aws_route53_zone.zone.name}"
      port        = "8501"
      protocol    = "HTTPS"
  ...
  condition {
    host_header {
      values = ["${var.prefix}${var.app_name2}${var.suffix}.${data.aws_route53_zone.zone.name}"]
    }
  }
}
...
resource "aws_alb_listener" "consul" {
  ...
  default_action {
    target_group_arn = aws_alb_target_group.consul.id
    type             = "forward"
  }
}
...
resource "aws_alb_listener_rule" "consul_http_rule" {
  listener_arn = aws_alb_listener.redirect_http_to_https.arn
  priority     = 100
...
resource "aws_alb_target_group" "consul" {
  name = "${var.environment}-${var.prefix}${var.app_name2}${var.suffix}-tg"
  ...
  tags = merge(
    var.extra_tags,
    map("Name", "${var.environment}-${var.prefix}${var.app_name2}${var.suffix}-tg"),
  )
}
